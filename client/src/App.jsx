...
export default function App() {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('update', ({ nearest, now }) => {
      if (!nearest) {
        setAc(null);
        return;
      }
  ...
  useEffect(() => {
    // fetch geolocation and subscribe
    if (!navigator.geolocation) {
      const fallback = { lat: 51.623842, lon: -0.269584 };
      setUserPos(fallback);
      socketRef.current && socketRef.current.emit('subscribe', { ...fallback, pollMs: 5000 });
      return;
    }
        ...
        <section className="info-card">
          ...
          {ac && (
            <>
              ...
              <p><strong>Ground speed:</strong> {ac.gs ? `${Math.round(ac.gs)} kt` : '—'}</p>
